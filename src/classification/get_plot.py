...
sys.path.insert(0, "../")
import pandas as pd
from src.classification.ploting_figures import MakePlots
...
class PerformanceMergerAndPlotter:
    ...
    def merge_documents(self):
        ...
        df_results_train = self.df_data[['algorithm', 'F1_cv', 'recall_cv', 'precision_cv', 'accuracy_cv']]
        df_results_train.columns = ["Algorithm", "F1", "Recall", "Precision", "Accuracy"]

        df_results_test = self.df_data[['algorithm', 'accuracy_val', 'precision_val', 'recall_val', 'f1_val']]
        df_results_test.columns = ["Algorithm", "Accuracy", "Precision", "Recall", "F1"]
